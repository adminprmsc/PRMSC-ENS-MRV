...
export class TehsilManagerService {
  ...
  async addWaterSystem(
    ...
    const findWhere: Record<string, unknown> = { tehsil: ct, village };
    if (settlement) findWhere.settlement = settlement;
    ...
    if (existingSystem) {
      ...
      existingSystem.pumpModel = this.operatorHelpers.coerceOptionalStr(
        data.pump_model,
      );
      existingSystem.pumpSerialNumber = this.operatorHelpers.coerceOptionalStr(
      ...
      try {
        if ('latitude' in data) {
          existingSystem.latitude = this.operatorHelpers.coerceOptionalFloat(
            data.latitude,
          );
        }
        if ('longitude' in data) {
          existingSystem.longitude = this.operatorHelpers.coerceOptionalFloat(
            data.longitude,
          );
        }
        existingSystem.depthOfWaterIntake =
          this.operatorHelpers.coerceOptionalFloat(data.depth_of_water_intake);
        existingSystem.heightToOhr = this.operatorHelpers.coerceOptionalFloat(
          data.height_to_ohr,
        );
        existingSystem.pumpFlowRate = this.operatorHelpers.coerceOptionalFloat(
          data.pump_flow_rate,
        );
      } catch (exc) {
      ...
      if (bmi !== null) existingSystem.bulkMeterInstalled = bmi;
      try {
        ...
        if ('pump_head' in data) {
          existingSystem.pumpHead = this.operatorHelpers.coerceOptionalFloat(
            data.pump_head,
          );
        }
        ...
        if ('time_to_fill' in data) {
          existingSystem.timeToFill = this.operatorHelpers.coerceOptionalFloat(
            data.time_to_fill,
          );
        }
      } catch (exc) {
  ...
  async addSolarSystem(
    ...
    if (existingSystem) {
      ...
      existingSystem.discoInfo = this.operatorHelpers.coerceOptionalStr(
        data.disco_info,
      );
      ...
      existingSystem.remarks = this.operatorHelpers.coerceOptionalStr(
        data.remarks,
      );
